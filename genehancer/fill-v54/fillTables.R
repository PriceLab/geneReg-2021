...
files <- c(elements="../incoming/v5.4/GeneHancer_elements.txt",
           tfbs="../incoming/v5.4/GeneHancer_TFBSs.txt",
           tissues="../incoming/v5.4/GeneHancer_tissues.txt",
           associations="../incoming/v5.4/GeneHancer_gene_associations.txt")
for(name in names(files)){
    file <- files[[name]]
    stopifnot(file.exists(file))
    }

db <- dbConnect(PostgreSQL(), user="trena", password="trena", dbname="gh54", host="khaleesi")
